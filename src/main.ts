...
function findNodesWithBoundVariables(): void {
  const selection = figma.currentPage.selection;

  if (selection.length === 0) {
    figma.notify("Select at least one node to scan for bound variables.");
    return;
  }
  ...
  if (matches.length === 0) {
    figma.notify("No bound variables found in selection.");
    return;
  }
...
function packPages(): void {
  ...
  figma.notify(
    `Packed ${frames.length} page${
      frames.length === 1 ? "" : "s"
    } into ${TEMP_PAGE_NAME}. Copy selection (Cmd/Ctrl+C).`
  );
}
...
function unpackPages(): void {
  ...
  if (tempPage === undefined) {
    console.log(
      `[TCC] ${TEMP_PAGE_NAME} not found, using current page: ${figma.currentPage.name}`
    );
  ...
  console.log(
    `[TCC] Found ${frames.length} top-level frames on page "${sourcePage.name}"`
  );
  ...
  const msg = `Unpacked ${createdPagesCount} page${
    createdPagesCount === 1 ? "" : "s"
  }.`;
  console.log(`[TCC] ${msg}`);
  figma.notify(msg);
}
